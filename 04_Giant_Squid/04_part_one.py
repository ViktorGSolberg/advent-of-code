...
def simulate_bingo(boards, numbers):
    ...
    no_bingo = True
    while no_bingo and len(picked_numbers) < len(numbers):
        picked_numbers = draw_next_number(picked_numbers, numbers)
        is_bingo, winning_numbers, winning_board = is_win(boards, picked_numbers)
        no_bingo = not is_bingo
    if is_bingo: return True, winning_numbers, winning_board, picked_numbers[-1], picked_numbers


def main():
    bingo_boards = generate_bingo_boards(bingo_board_numbers)
    result, winning_numbers, winning_board, last_picked_number, picked_numbers = simulate_bingo(bingo_boards, bingo_draw_numbers)
    sum_unmarked_numbers = find_sum_of_unmarked_numbers(winning_board, picked_numbers)
    print(sum_unmarked_numbers * last_picked_number)
